...
@app.callback(
    ...
    [Input(component_id='select-year', component_property='enabled'), 
    Input(component_id='dropdown-statistics', component_property='enabled')])


def update_output_container(data, input_year):
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        ...
        R_chart2  = dcc.Graph(figure=px.line(yearly_rec,
                                            x='Vehicle',
                                            y='Automobile_Sales',
                                            title='Average number of vehicles sold vehicles by vehicle type')
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        unemploy_rate = recession_data.groupby("Vehicle_Type")[["unemployment_rate","Automobile_Sales"]].sum().reset_index()
        R_chart4 = dcc.Graph(
            figure=px.bar(
                data=unemploy_rate,
                x='Vehicle_Type',
...
# Plot 2 Total Monthly Automobile sales using line chart.
        auto_sales_month = data.groupby("Month")["Automobile_Sales"].sum().reset_index()
        Y_chart2 = dcc.Graph(
            figure=px.line(
                data=auto_sales_month,
                x='Month',
                y='Automobile_Sales',
                title='Total Automobile sales per Month'
            )
        )
        ...
        Y_chart3 = dcc.Graph( 
            ...
            # Total 4 Advertisement Expenditure for each vehicle using pie chart
